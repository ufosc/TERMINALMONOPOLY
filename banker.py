        ...
        while True:
            ...
            for reader in readers:
                if reader is server:
                    ...
                    connected_clients[player] = (
                        address  # store address of client in dict
                    )
                    ...
                    if not data:  # No data indicates disconnect
                        ...
                        if len(connected_clients) == 0:
                            ...
                            return
            print(
                f"{s.set_cursor_str(90, 0)}{s.COLORS.backBLUE}Time passed since last command: {timer}. ",
                flush=True,
                end=f"\r{s.COLORS.RESET}",
            )
            timer += 1
    ...
    if data.decode() == "request_board":
        ...
        if gm.game_exists("Battleship") == False:
            ...
            if len(gm.get_game_by_name("Battleship")) > 1:
                ...
                net.send_message(
                    client, gm.display_games(name="Battleship", player_name=client_name)
                )
...
def handshake(client_socket: socket.socket, handshakes: list):
    ...
    if message == "Connected!":
        ...
        if (
            ...
            or clients[len(clients) - 1].name == "PAD ME PLEASE!"
        ):
