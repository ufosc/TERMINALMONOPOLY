...
class Ship:
    ...
    def in_bounds(self, x, y) -> bool:
        return (
            x + self.size[0] <= ss.cols
            and y + self.size[1] <= ss.rows
            and x >= 2
            and y >= 1
        )
...
class BattleshipGame:
    ...
    def print_explosion(self, x: int, y: int) -> str:
        text = ""
        text += (
            s.set_cursor_str(x, y)
            + random.choice([s.COLORS.ORANGE, s.COLORS.RED])
            + random.choice(["░", "▒", "▓"])
        )
        return text
    ...
    def popup(self, message: str, color: str = s.COLORS.WHITE) -> str:
        ...
        for i in range(len(outline)):
            ...
            if 0 < i < 4:
                ...
                p += (
                    s.set_cursor_str(27, 5 + i)
                    + message[(i - 1) * 26 : (i - 1) * 26 + 26]
                )
        return p
            ...
            while ship.x == -1:
                ...
                if ship.in_bounds(x, y):
                    if self.is_overlapping(x, y, ship.size, player_num) == None:
                        ...
                        return self.popup(
                            f"Error! This ship is on    another ship. Recall this ship size is {ship.size}",
                            s.COLORS.RED,
                        )
                else:
                    return self.popup(
                        f"Error! Out of bounds. Max x is {ss.cols-ship.size[0]}. Max y is {ss.rows-ship.size[1]}",
                        s.COLORS.RED,
                    )
                ...
                if 0 <= move < len(self.ships[player_num]):
                    ...
                    while new_ship.x == -1:
                        ...
                        if new_ship.in_bounds(x, y):
                            if (
                                self.is_overlapping(x, y, new_ship.size, player_num)
                                == None
                            ):
                                new_ship.x = x
                                new_ship.y = y
                                ...
                                return self.popup(
                                    f"Error! This ship is on    another ship. Recall this ship size is {new_ship.size}",
                                    s.COLORS.RED,
                                )
                        else:
                            return self.popup(
                                f"Error! Out of bounds. Max x is {ss.cols-new_ship.size[0]}. Max y is {ss.rows-new_ship.size[1]}",
                                s.COLORS.RED,
                            )
                    return_board = self.get_ship_board(player_num)
                    done = input(
    ...
    def attack(self):
        while len(self.ships) > 0:  # not correct final logic, temporary
            ...
            if x < 2 or x > 74:
                self.popup(
                    "Error! Out of bounds. Please enter a valid          coordinate (2-74) inclusive.",
                    s.COLORS.RED,
                )
                continue
            ...
            if y < 1 or y > 19:
                self.popup(
                    "Error! Out of bounds. Please enter a valid          coordinate (1-19) inclusive.",
                    s.COLORS.RED,
                )
                continue
