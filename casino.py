...
def module(active_terminal):
    ...
    while True:
        ss.overwrite(
            player.COLORS.RESET
            + "\rSelect a game through typing the associated command."
            + " " * 20
        )
        ss.update_quadrant(
            active_terminal,
            ...
            + "CASINO MODULE"
            + "─" * 31
            + f"\n$ BALANCE = {player.balance} $\nSelect a game by typing the command."
            + get_submodules()
            + "\n☒ Exit (e)",
        )
        if wrong:
            ss.overwrite(
                player.COLORS.RESET
                + player.COLORS.RED
                + "\rGame does not exist. Refer to the list of games."
            )
        ...
        try:
            ...
            if bet < 0:
                ss.overwrite(
                    player.COLORS.RESET
                    + player.COLORS.RED
                    + "\rYou need to bet a number above 0. (0 to cancel)"
                    + " " * 20
                )
            else:
            ...
            ss.overwrite(
                player.COLORS.RESET
                + player.COLORS.RED
                + "\rType in a valid number for your bet. (0 to cancel)"
                + " " * 20
            )
