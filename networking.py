HEADERSIZE = (
    10  # Max length of the header, meaning the max length of the message is 10^10 bytes
)
import socket
...
def format_message(text: str) -> tuple:
    ...
    """
    FOOTERSIZE = (
        16 - ((len(text) + HEADERSIZE) % 16)
    ) % 16  # Padding to make sure the message is a multiple of 16 bytes
    ...
    return (
        bytes(header, "utf-8"),
        msg + bytes((" " * FOOTERSIZE), "utf-8"),
    )  # Return the header and the message with padding
