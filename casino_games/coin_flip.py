...
def play(active_terminal: Terminal, player, bet: int):
    """
    Coin Flip
    ...
    choice = input(c.backYELLOW + c.BLACK + f"\r{player.name}: Heads or Tails? (h/T) ")
    ss.overwrite("\r" + " " * 40)
    flip = random.choice(['heads', 'tails'])

    # TODO - Make the animation asynchronous.
    active_terminal.update(header + "\n" + g['coin_flip_heads'])
    sleep(0.2)
    active_terminal.update(header + "\n" + g['coin_flip_middle'])
    sleep(0.2)
    ...
    if choice.lower() == "h" and flip == 'heads':
        input(c.backYELLOW + c.BLACK + f"\r{player.name}: You got heads!")
        ss.overwrite("\r" + " " * 40)
        score[0] += 1
    elif choice.lower() == "h" and flip == 'tails':
        input(c.backYELLOW + c.BLACK + f"\r{player.name}: You got tails...")
        ss.overwrite("\r" + " " * 40)
    elif flip == 'heads':
        input(c.backYELLOW + c.BLACK + f"\r{player.name}: You got heads...")
        ss.overwrite("\r" + " " * 40)
