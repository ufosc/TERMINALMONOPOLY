...
def initialize():
    ...
    try:
        ...
        n = input(
            COLORS.RED
            + "Handshake failed. Type 'exit' to quit or press enter to try again.\n"
            + COLORS.RESET
        )
        if n == "exit":
            quit()
...
def handshake(sock: socket.socket) -> str:
    ...
    if message == "Welcome to the game!":
        ...
        s.print_w_dots(
            COLORS.RED + "Handshake failed. Reason: Connected to wrong foreign socket."
        )
...
def calculate() -> None:
    ...
    while True:
        ...
        if player_equation == "e":
            ss.update_quadrant(
                active_terminal, calculator_terminal_response(2), padding=True
            )
            break
        elif player_equation == "error":
            ss.update_quadrant(
                active_terminal, calculator_terminal_response(3), padding=True
            )
        else:
...
def game_input() -> None:
    ...
    try:
        ...
        ss.overwrite(
            COLORS.RED
            + "Something went wrong. The Banker may not be ready to start the game.\n"
        )
        print(e)
    ...
    while stdIn != "exit":
        ...
        if stdIn.startswith("help"):
            if (
                len(stdIn) == 6
                and stdIn[5].isdigit()
                and 2 >= int(stdIn.split(" ")[1]) > 0
            ):
                ss.update_quadrant(
                    active_terminal,
                    text_dict.get(stdIn if stdIn != "help 1" else "help"),
                    padding=True,
                )
            else:
                ss.update_quadrant(active_terminal, text_dict.get("help"), padding=True)
                ss.overwrite(
                    COLORS.RED + "Incorrect syntax. Displaying help first page instead."
                )
        elif stdIn == "game":
            game_input()
            stdIn = ""
            ...
            ss.update_quadrant(
                active_terminal,
                f"Cash on hand: {balance}".center(ss.cols),
                padding=True,
            )
        elif stdIn == "list":
            ...
            if (
                ...
                ss.overwrite(
                    COLORS.RESET
                    + COLORS.RED
                    + "Include a number between 1 and 4 (inclusive) after 'term' to set the active terminal."
                )
            ...
            # ss.overwrite('\n' + ' ' * ss.WIDTH)
            ss.overwrite(
                COLORS.RED + "Invalid command. Type 'help' for a list of commands."
            )
