...
def refresh_board():
    ...
    for i in range(40):
        ...
        print(
            COLORS.backBLACK
            + color
            + f"\033[{board.locations[i].x};{board.locations[i].y}H{i}"
            + backcolor
            + " " * (4 + (1 if i < 10 else 0))
        )
        ...
        if board.locations[i].owner == -3:  # If community chest
            print(end=COLORS.RESET)
            print(
                f"\033[{board.locations[i].x + 1};{board.locations[i].y}H"
                + COLORS.COMMUNITY
                + "█" * 6
            )
            print(
                f"\033[{board.locations[i].x + 2};{board.locations[i].y}H"
                + COLORS.COMMUNITY
                + "▀" * 6
            )
        ...
        if board.locations[i].houses > 0:  # If there are houses
            print(end=COLORS.RESET)
            print(
                f"\033[{board.locations[i].x+2};{board.locations[i].y+1}H"
                + COLORS.GREEN
        ...
        if board.locations[i].houses == 5:  # If there is a hotel
            print(end=COLORS.RESET)
            print(
                f"\033[{board.locations[i].x+2};{board.locations[i].y+5}H"
                + COLORS.RED
                + "▀"
            )
    ...
    for i in range(num_players):
        ...
        print(
            color
            + f"\033[{board.locations[players[i].location].x+1};{board.locations[players[i].location].y+1+i}H{token}"
        )
        ...
        try:
            ...
            if location.owner > -1:  # if the location is owned
                ...
                status.append(
                    f"Current owner: "
                    + color
                    + f"Player{location.owner}"
                    + COLORS.RESET
                )
                status.append(f"Houses: {location.houses}")
            if (
                location.rent != 0
            ):  # if location could be owned and is not a utility or railroad
                ...
                location.owner >= -2 and location.rent == 0
            ):  # if is a railroad or utility
                status.append(f"{location.color}=== {location.name} ===")
                status.append(f"Purchase Price: {location.purchasePrice}")
...
def refresh_h_and_s():
    ...
    for i in range(len(history)):
        print(
            f"\033[{i+4};81H" + history[i] if i < len(history) else "", end=COLORS.RESET
        )
    ...
    for i in range(len(sorted_players)):
        if sorted_players[i].order != -1:
            ...
            print(
                color
                + f"\033[{31+i};122H{sorted_players[i].order} - ${sorted_players[i].cash}",
                end=COLORS.RESET,
            )
...
def housing_logic(p: Player):
    ...
    try:
        ...
        print(
            f"\033[42;0"
            + COLORS.RED
            + f"Invalid input, please enter a number in {p.properties}",
            end=COLORS.RESET,
        )
        flag = False
    if flag and not exit:
        if not propertyid in p.properties:
            ...
            if (
                family == COLORS.CYAN
                or family == COLORS.LIGHTBLACK
                or board.locations[propertyid].name.startswith("Electric")
            ):
...
def player_roll(num_rolls):
    ...
    if players[turn].order != -1:  # If player is not bankrupt
        ...
        if players[turn].jail == False:
            if (
                players[turn].location + dice[0] + dice[1]
            ) > 39:  # checks if player passed go
                update_history(
        ...
        while not done_moving_around:
            ...
            if board.locations[players[turn].location].owner < 0:
                match board.locations[players[turn].location].owner:
                    ...
                    case -3:  # community chest
                        ...
                        if (
                            old_loc > new_loc
                            and new_loc != 10
                            and new_loc != players[turn].location - 3
                ...
                if (
                    board.locations[cl].name == "Electric Company"
                    or board.locations[cl].name == "Water Works"
                ):
                    rent *= dice[0] + dice[1]

                if (
                    card != ""
                ):  # if player collected a specific chance card and moved afterwards on same turn
...
while True:
    ...
    if players[turn].cash > 0:
        ...
        while (
            choice != "e"
        ):  # @TODO remove soon! players should not be able to do these actions during gameboard screen
            if choice == "e":
                pass
            elif choice == "p":
