...
def notification(msg: str, n: int, color: str) -> str:
    ...
    for i in range(len(outline)):
        ...
        if 0 < i < 4:
            ...
            p += (
                set_cursor_str(x + 2, y + i) + message[(i - 1) * 26 : (i - 1) * 26 + 26]
            )
    writeto += p
    return writeto
...
def update_quadrant(n: int, data: str, padding: bool = True):
    ...
    if data:
        ...
        if len(line_list) > rows and padding:
            line_list = line_list[
                :rows
            ]  # Truncate if necessary bc someone might send a long string
        for i in range(len(line_list)):
            ...
            print(
                line_list[i][:cols]
                if len(line_list[i]) > cols and padding
                else line_list[i]
            )  # Truncate if necessary bc someone might send a long string
...
def overwrite(text: str = ""):
    ...
    set_cursor(0, INPUTLINE)
    print(
        f"\033[1A\r{COLORS.RESET}{text}",
        end=" " * (WIDTH - len(text) + 3) + "\n" + " " * (WIDTH + 3) + "\r",
    )
...
def calibrate_screen(type: str) -> None:
    ...
    if current_os == "Darwin":
        ...
        print(
            'Please use Ctrl + "Command" + "+" or Ctrl + "Command" + "-" to zoom in/out and ensure everything is visible. Press enter to continue to scaling screen.'
        )
    else:
        # Print out instructions for Linux/Windows users
        print(
            'Please use "Ctrl" + "-" or "Ctrl" + "+" to zoom in/out and ensure everything is visible. Press enter to continue to scaling screen.'
        )
    print(
        "Proper scaling should only displays 4 cross that marks the corners of the board."
    )
    print(
        "If you are having trouble with scaling, try entering r to reset the display."
    )
    ...
    if type == "gameboard":
        ...
        print(
            f"\033[36;0H" + "Press enter to play or enter r to reset the display.",
            end="",
        )
        scaling_test = input()
        while scaling_test != "":
            if scaling_test == "r":
                ...
                print(
                    f"\033[36;0H"
                    + "Press enter to play or enter r to reset the display.",
                    end="",
                )
            scaling_test = input()
    elif type == "player":
        ...
        print(
            f"\033[44;0H" + "Press enter to play or enter r to reset the display.",
            end="",
        )
        scaling_test = input()
        while scaling_test != "":
